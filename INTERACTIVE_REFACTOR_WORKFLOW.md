...
8.  **终止点 (STOP)**:
    ...
    * **显式指令**: "请确认业务规则是否完整、范围与分级是否正确。回复 **[确认]** 进入策略设计阶段，或补充业务上下文。"
    * **立即停止生成，等待用户回复。**
...
**首先，执行阶段 0（范围确认）**：
1. 输出范围确认询问，要求用户明确指定重构的类/方法和目标。
2. **立即停止回复**，等待用户明确回复。
3. **禁止**在用户回复前执行任何分析、测试或代码操作。
