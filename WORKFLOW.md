...
2.  **撰写技术设计文档**：基于下方的《技术设计文档模板》编写。在此阶段**绝不要写实现代码**。
3.  **用户评审**：提交技术设计文档给用户。
    * 用户有反馈 -> 修改文档。
    * 用户批准 -> 进入阶段 3。
...
2.  **设计锚点锁定 (Design Anchoring)**：为了防止写代码时"天马行空"或遗忘设计要求，**你必须在编写每个核心逻辑块之前，强制回顾技术设计文档。**
    * **强制动作**：在代码注释中显式引用技术设计文档章节。
    * *示例*: `// [设计文档 3.2] 策略模式实现：此处遵循文档定义的扩展性要求`
3.  **规范与编译检查 (关键)**：
