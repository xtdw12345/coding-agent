...
* **逻辑规格**（每个 LOGIC-ID 必须包含完整的设计理由）：

    #### `[LOGIC-01]` 认证逻辑
    | 维度 | 内容 |
    |-----|------|
    | **现状** | 当前系统允许任何状态的用户访问资源 |
    | **问题** | 非活跃用户（已禁用/已注销）仍能访问敏感数据，存在安全风险 |
    | **方案** | 在访问资源前检查用户状态，非活跃用户返回 403 |
    | **预期行为** | `user.status == INACTIVE` → 返回 403 Forbidden |

    #### `[LOGIC-02]` 重试机制
    | 维度 | 内容 |
    |-----|------|
    | **现状** | 外部 API 调用失败时直接抛出异常 |
    | **问题** | 网络抖动导致大量请求失败，用户体验差 |
    | **方案** | 引入指数退避重试，最多重试 3 次 |
    | **预期行为** | 失败 → 等待 1s → 重试 → 等待 2s → 重试 → 等待 4s → 重试 → 最终失败则抛出 |

    > ⚠️ **每个 LOGIC-ID 必须填写完整的 现状→问题→方案→预期行为**
    > Agent 实现时必须理解"为什么要这样改"，而不仅仅是"改成什么样"
