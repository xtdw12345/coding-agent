...
2.  **强制中断 (Mandatory Stop)**: 在阶段 1 (范围与分级)、阶段 2 (设计) 和 阶段 3 (计划) 产出后，**必须立即停止生成**，请求用户评审。
...
7.  **终止点 (STOP)**:
    * 输出消息："*任务 ID `{TaskID}` 已建立。技术栈与文件范围已探测。任务级别：X级。*"
    * **显式指令**: "请确认范围与分级。回复 **[确认]** 进入设计阶段，或补充上下文。"
    * **立即停止生成，等待用户回复。**
