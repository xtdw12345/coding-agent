...
│  // ✅ Good: 只在关键处添加说明                                          │
│  public BigDecimal calculateTotal(Order order) {                        │
│      // Apply VIP discount before shipping fee per business rule BR-02  │
│      ...                                                                │
│  }                                                                      │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
#### 4.8 熔断机制 (Circuit Breaker)

**当同一步骤连续失败多次时，必须触发熔断：**

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ⚠️ 熔断触发条件：同一重构步骤连续失败 3 次                              │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  必须立即停止自动执行，输出：                                           │
│  1. 已尝试的方案及各自失败原因                                         │
...
**请选择下一步**:
- [ ] **[继续]** 我有新思路，继续尝试
- [ ] **[回滚]** 回滚到 STEP-XX-1 的状态
- [ ] **[跳过]** 跳过此步骤，记录为 SKIPPED
- [ ] **[中止]** 终止重构任务
```
...
| 用户决定 | Agent 必须执行的操作 |
|----------|----------------------|
| **[采纳]** | 1. 更新 `strategy.md` 中对应 STEP 的设计<br>2. 更新 `plan.md` 中的设计摘要<br>3. 按新方案执行<br>4. 在进度汇报中注明"方案已变更" |
| **[放弃]** | 1. 在 `strategy.md` 中标记该 STEP 为"已跳过"并说明原因<br>2. 在 `plan.md` 中标记为 `- [~] STEP-XX: (已跳过)`<br>3. 继续执行下一个步骤 |
| **[讨论]** | 1. 与用户讨论替代方案<br>2. 达成一致后按 [采纳] 或 [放弃] 处理 |

**文档更新示例**：

```markdown
# strategy.md 更新示例（采纳变更）

### STEP-03: 提取 calculateShipping() - [已变更]

> ⚠️ **方案变更记录**
> - 变更时间: 2024-01-15
> - 原因: 发现 shipping 逻辑与 inventory 模块强耦合，无法简单提取
> - 新方案: 改为内联优化，不提取独立方法

**原设计**: 提取为独立的 ShippingCalculator 类
**新设计**: 保持在 OrderService 中，仅重构内部结构

[更新后的重构前/后代码...]
```

```markdown
# strategy.md 更新示例（放弃步骤）

### STEP-03: 提取 calculateShipping() - [已跳过]

> ⚠️ **步骤跳过记录**
> - 跳过时间: 2024-01-15
> - 原因: 发现 shipping 逻辑与 inventory 模块强耦合，重构风险过高
> - 用户决定: 保持原状，作为后续独立任务处理
> - 后续建议: 先重构 inventory 模块，再处理此部分
```
...
| `[暂停]` | 阶段 4 | 暂停执行，保存进度 |
| `[回滚]` | 阶段 4 | 回滚到上一个步骤 |
| `[采纳]` | 阶段 4（方案变更） | 同意变更请求，按新方案执行 |
| `[放弃]` | 阶段 4（方案变更） | 放弃该步骤重构，保持原代码 |
| `[讨论]` | 阶段 4（方案变更） | 需要进一步讨论其他方案 |
| `[中止]` | 任何阶段 | 立即中止整个任务 |
...
- ❌ **发现需要变更方案时擅自决定，不通知用户**
- ❌ **方案变更后不更新 strategy.md 和 plan.md**
- ❌ 在进度汇报中跳过"设计一致性"声明
...
- ❌ **为类或方法添加完整的文档注释**（Javadoc/docstring）
- ❌ **使用中文编写代码注释**（注释必须使用英文）
