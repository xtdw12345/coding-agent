...
```
❌ 禁止：在阶段 2 分析完成后，未等用户确认就进入阶段 3
❌ 禁止：在阶段 3 生成 design.md 后，未等用户 Review 就进入阶段 4
❌ 禁止：在阶段 4 生成 plan.md 后，未等用户 Review 就开始编写代码
❌ 禁止：用户说"帮我写测试"就直接开始写代码，跳过分析和设计阶段
```

### 禁止忘记更新任务状态

```
❌ 禁止：完成一个测试用例后，不更新 plan.md 中对应的 checkbox
❌ 禁止：批量完成多个任务后才一次性更新 plan.md
❌ 禁止：编写代码时不同步更新进度
```

**强制要求**：在阶段 5 编写代码时，每完成一个任务项（无论大小），**必须立即**将 plan.md 中对应的 `[ ]` 改为 `[x]` 并添加完成时间戳。这是不可跳过的步骤。

### 必须等待用户的明确指令

只有收到以下类型的用户回复后，才能进入下一阶段：

| 当前阶段 | 需要等待的用户回复示例 |
|----------|------------------------|
| 阶段 2 完成 | "确认"、"可以"、"继续"、"同意"、"OK" |
| 阶段 3 完成 | "Review 通过"、"确认"、"没问题"、"可以继续" |
| 阶段 4 完成 | "开始编写"、"确认"、"执行"、"开始吧" |
...
阶段 1: 准备 → 阶段 2: 分析确认 → 阶段 3: 测试设计 → 阶段 4: 实施计划 → 阶段 5: 编写代码
   ↓            ↓                  ↓                ↓                ↓
 自动执行    等待用户确认     生成 design.md     生成 plan.md      执行并实时更新 plan.md
                              等待用户 Review    等待用户 Review    ⚠️ 每完成一项立即更新状态
```

> ⚠️ **阶段 5 特别提醒**：编写代码时，每完成一个任务必须**立即**将 plan.md 中对应的 `[ ]` 改为 `[x]`，禁止批量更新。
...
> ⛔ **必须停止**
>
> 输出以上内容后，**立即停止**，不要继续执行任何操作。
> 等待用户回复确认后，才能进入阶段 3。
> **禁止**在同一轮对话中继续生成 design.md。
...
> ⛔ **必须停止**
>
> 生成 design.md 并输出以上内容后，**立即停止**，不要继续执行任何操作。
> 等待用户 Review 通过后，才能进入阶段 4。
> **禁止**在同一轮对话中继续生成 plan.md 或编写测试代码。
...
> ⛔ **必须停止**
>
> 生成 plan.md 并输出以上内容后，**立即停止**，不要继续执行任何操作。
> 等待用户明确指示"开始编写"或类似确认后，才能进入阶段 5。
> **禁止**在同一轮对话中开始编写任何测试代码。
...
>
> 本阶段的核心原则是：**写一个，更新一个**。
> 每完成一个任务项，**必须立即**更新 plan.md，这是不可省略的步骤。

#### 5.1 按计划逐步实施

用户确认后，按照 plan.md 中的清单逐个编写测试。

**⚠️ 强制执行：每完成一个任务项，必须立即执行以下操作：**

1. 将 plan.md 中对应的 `[ ]` 改为 `[x]`
2. 在该行末尾添加完成时间戳，格式：`✅ YYYY-MM-DD HH:mm`
3. 确认文件已保存后，再继续下一个任务

> ⛔ **禁止行为**
>
> 禁止连续编写多个测试用例后才批量更新 plan.md。
> 禁止在阶段 5 结束后才统一标记所有任务为完成。
