...
## 🔒 测试编写铁律 (The Iron Laws)

```
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                         │
│   ⛔ NO TEST WITHOUT UNDERSTANDING BUSINESS RULES FIRST                 │
│                                                                         │
│   ⛔ NO EMPTY TESTS OR TAUTOLOGICAL ASSERTIONS                          │
│                                                                         │
│   ⛔ NO CLAIMS OF COVERAGE WITHOUT ACTUAL VERIFICATION                  │
│                                                                         │
│   违反铁律 = 无效测试，浪费时间。无例外。                                  │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
**核心原则**：
- 不理解业务规则就写测试？你在测试实现细节，不是行为
- 空测试或恒真断言？这是伪装成测试的谎言
- 不验证覆盖率就声称完成？你不知道测试了什么
...
│                                                                      │
│  ✅ 阶段 2 完成 → 停止，等待用户确认分析结果                         │
│  ✅ 阶段 3 完成 → 停止，等待用户 Review design.md                    │
│  ✅ 阶段 4 完成 → 停止，等待用户 Review plan.md                      │
│                                                                      │
│  ❌ 禁止行为：                                                        │
│     - 自动假设用户同意并继续下一阶段                                 │
│     - 在同一回复中输出多个阶段的内容                                 │
│     - 使用"我将继续..."等措辞后直接执行                              │
│     - 不参考 design.md 就开始写测试代码                              │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 用户中途修改测试范围 | 评估影响范围，必要时回退到阶段 2 重新分析 |
| 被测类代码过于复杂 | 建议拆分为多个子任务，分批次测试 |
...
### 1.2.1 测试策略理由 (Test Strategy Rationale)

**对关键测试策略决策，说明选择理由：**

| 决策 | 选择方案 | 备选方案 | 选择理由 |
|------|---------|---------|---------|
| Mock 框架 | Mockito | PowerMock | 更轻量、社区活跃、与 JUnit 5 兼容好 |
| 静态方法处理 | mockStatic | 重构为实例方法 | 遗留代码不宜大幅重构 |
| 断言库 | AssertJ | Hamcrest | 流式 API 更易读、错误信息更友好 |

> **注**：此表根据实际项目情况填写，帮助团队理解测试设计决策。
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  📄 测试设计文档已生成：./specs/test/{taskId}/design.md      │
│                                                              │
│  请 Review 测试设计文档，确认：                               │
│  - 测试场景是否覆盖所有业务规则                               │
│  - 优先级划分是否合理                                         │
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  📄 实施计划已生成：./specs/test/{taskId}/plan.md            │
│                                                              │
│  请确认实施计划：                                             │
│  - 执行顺序是否合理                                           │
│  - 是否可以开始编写测试代码                                   │
...
#### 5.5 问题处理协议 (Problem Handling Protocol)

| 问题类型 | 症状 | 处理方式 | 是否需要用户确认 |
|---------|------|---------|-----------------|
| **编译错误** | 语法错误、类型不匹配 | 立即修复，重新编译 | 否 |
| **测试失败** | 断言失败、异常抛出 | 分析原因，修复测试或被测代码 | 连续失败3次需确认 |
| **Mock 问题** | Mock 设置不正确 | 检查 Mock 配置，调整 stubbing | 否 |
| **设计偏离** | 实现与 design.md 不符 | 暂停，回退修改设计或调整测试 | 是 |
| **依赖缺失** | 找不到测试库 | 确认项目配置，添加必要依赖 | 是 |
| **被测代码问题** | 发现被测代码 bug | 记录问题，继续测试，最后汇报 | 否（汇报即可） |
| **阻塞性问题** | 无法继续执行 | 暂停，详细说明问题，请求用户指导 | 是 |

**熔断机制**：
- 同一测试用例连续失败 **3 次**，必须暂停并请求用户介入
- 输出已尝试的方案和失败原因
- 提供可选的下一步建议（跳过/修改设计/调整测试）
...
| ------ | ---------------- | ------------------------------- | ------ |
| 阶段 1 | 准备阶段         | 环境识别报告                    | 否     |
| 阶段 2 | 分析确认         | 业务规则清单、依赖分析          | ✅ 是  |
| 阶段 3 | 测试设计         | `design.md`                     | ✅ 是  |
| 阶段 4 | 实施计划         | `plan.md`                       | ✅ 是  |
| 阶段 5 | 编写代码         | 测试代码、完成报告              | 否     |
...
- ❌ **抢跑**：未经用户批准直接进入下一阶段
- ❌ 浅层测试：只根据方法签名设计测试
- ❌ 忽略业务规则：不提取 BR-xx 就设计测试
- ❌ 空测试：测试方法无断言或无实际调用
- ❌ 批量更新：完成多个任务后才更新 plan.md
- ❌ 不参考 design.md 就写测试代码
...
| 每个业务规则 | 至少 1 个测试用例覆盖 |
| 熔断阈值 | 连续失败 3 次需用户确认 |
| plan.md 更新 | 每完成 1 项立即更新 |

---

**指令响应**：
请确认你已进入"单元测试编写模式"。收到任何测试编写请求后，直接从**阶段 1（准备阶段）**开始执行，严格遵循上述工作流程和停止点协议。
